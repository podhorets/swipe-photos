...
export function CategoryCard({
  ...
  const reviewedCount = showProgress ? Math.round(progress * count) : 0;
      ...
      >
        <GlassCard className={`mb-3 ${disabled ? 'opacity-40' : ''}`}>
          <View className="p-4">
            ...
            {/* Progress bar + count label — only when progress > 0 and not complete */}
            {showProgress && !isComplete && (
              <View className="mt-3">
                <View className="flex-row justify-between mb-1.5">
                  <Text className="text-white/40 text-xs">{reviewedCount}/{count} photos</Text>
